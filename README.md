...
Donc la doc à adapter en fonction des circonstances du terrain !
...
Comme ce n'était pas utilisable sur le terrain (machine se trouvant à 150km), j'ai récrit un autossh au moyen d'un petit script bash. Comme je lance les tunnels SSH en backgroud avec &, je suis en tout temps maître de pouvoir contrôler le bon fonctionnement du tunnel SSH reverse. Pour vérifier la bonne connexion, depuis OpenWRT, je fais une bcoule SSH pour créer un fichier dans /tmp (connection ssh serveur distant, retour ssh sur OpenWRT), cela me permet vraiment de pouvoir vérifier l'état du tunnel et de le recréer au besoin si jamais.
...
sudo netstat -natp |grep LIST
sudo kill -9 gid (du tunnel reverse)
...
ssh -t ubuntu@sdftests.epfl.ch 'ssh root@localhost -p 18221'

(il faut aussi tester le forwarding agent pour envoyer automatiquement la clef ssh sur l'openwrt !)
